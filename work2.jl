...
findall(x->x.condition == ABES.carrier, pop)

dists = load("data/duration_dists.jld")
dists = [dists["F_E"], dists["F_I"], dists["F_C"]]
α = vcat(fill(0.15, 30), fill(0.1, 30), fill(0.055, 30), fill(0.075, 30), fill(0.06, 30), fill(0.075, 30),fill(0.065, 30), fill(0.075, 30))
# α = 0.13
param_probs = [0.01, 0.5, 0.015, 0.025]

model1 = ABES.AgentModel(ws_graph, α, param_probs..., dists..., 5, 5, 5, pop)
@time df_results = ABES.simulate(model1, 239)
#
# first(df_results, 60)
...
CSV.write("data/results_with_delta_fixed5.csv", df_results)
